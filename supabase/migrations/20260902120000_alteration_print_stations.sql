-- ============================================================================
-- Alterations print stations: reliable Active/Standby slips for the workshop
-- ============================================================================
-- The workshop runs its own Wi-Fi receipt printer (TA POS TA-900UWB) and its own
-- pair of Android tablets. This branch is deliberately separate from the
-- tailoring branch created in migration 81: separate stations, separate lease,
-- separate queue. Neither branch can claim the other's jobs.
--
-- Shared with migration 81 (created there, reused here, never redefined):
--   private.print_station_devices, private.print_station_leases,
--   private.hash_print_station_secret, private.require_active_admin,
--   and every public.print_jobs column, policy and grant.

-- --------------------------------------------------------------------------
-- Admit the new branch into the shared station tables
-- --------------------------------------------------------------------------

ALTER TABLE private.print_station_devices
  DROP CONSTRAINT IF EXISTS print_station_devices_branch_check;

ALTER TABLE private.print_station_devices
  ADD CONSTRAINT print_station_devices_branch_check
  CHECK (branch IN ('tailoring', 'alterations'));

ALTER TABLE private.print_station_leases
  DROP CONSTRAINT IF EXISTS print_station_leases_branch_check;

ALTER TABLE private.print_station_leases
  ADD CONSTRAINT print_station_leases_branch_check
  CHECK (branch IN ('tailoring', 'alterations'));

-- Seed the lease row so the very first heartbeat only has to lock it, exactly
-- as migration 81 seeds the tailoring row.
INSERT INTO private.print_station_leases (branch)
VALUES ('alterations')
ON CONFLICT (branch) DO NOTHING;

-- Claim and recovery scans mirror the tailoring partial indexes so the queue
-- stays O(log n) once both branches share the table.
CREATE INDEX IF NOT EXISTS idx_print_jobs_alterations_claimable
  ON public.print_jobs (next_attempt_at, created_at, id)
  WHERE branch = 'alterations' AND status = 'pending';

CREATE INDEX IF NOT EXISTS idx_print_jobs_alterations_expired
  ON public.print_jobs (job_lease_expires_at, id)
  WHERE branch = 'alterations'
    AND status = 'printing'
    AND job_token IS NOT NULL;

-- public.print_jobs RLS from migration 81 already covers this branch correctly:
-- the INSERT policy is fabrics-only, so alteration slips can only be queued
-- through the SECURITY DEFINER RPC below, and the SELECT policy exposes a
-- non-fabrics branch to admins alone.

-- --------------------------------------------------------------------------
-- Private helpers
-- --------------------------------------------------------------------------

CREATE OR REPLACE FUNCTION private.require_alterations_print_station(
  p_station_id UUID,
...
CREATE OR REPLACE FUNCTION private.alterations_retry_delay(p_attempt_count INTEGER)
RETURNS INTERVAL
LANGUAGE SQL
IMMUTABLE
STRICT
...
CREATE OR REPLACE FUNCTION private.recover_alterations_station_jobs(
  p_station_id UUID,
  p_generation BIGINT,
  p_reason TEXT
)
...
REVOKE ALL ON FUNCTION private.require_active_admin()
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.require_alterations_print_station(UUID, TEXT)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.alterations_retry_delay(INTEGER)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.recover_alterations_station_jobs(UUID, BIGINT, TEXT)
  FROM PUBLIC, anon, authenticated;
...
CREATE OR REPLACE FUNCTION private.create_alterations_print_station_impl(
  p_name TEXT,
  p_priority INTEGER
)
RETURNS JSONB
...
CREATE OR REPLACE FUNCTION private.rotate_alterations_print_station_secret_impl(
  p_station_id UUID
)
RETURNS JSONB
LANGUAGE plpgsql
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION USING
      ERRCODE = 'P0002',
      MESSAGE = 'alterations_print_station_not_found';
  END IF;

  SELECT l.generation, l.station_id = p_station_id
  INTO v_generation, v_is_holder
  FROM private.print_station_leases AS l
  WHERE l.branch = 'alterations'
  FOR UPDATE;

  IF v_is_holder THEN
    PERFORM private.recover_alterations_station_jobs(
      p_station_id,
      v_generation,
...
CREATE OR REPLACE FUNCTION private.set_alterations_print_station_enabled_impl(
  p_station_id UUID,
  p_enabled BOOLEAN
)
RETURNS JSONB
...
CREATE OR REPLACE FUNCTION private.release_alterations_print_station_lease_impl(
  p_station_id UUID
)
RETURNS JSONB
LANGUAGE plpgsql
...
CREATE OR REPLACE FUNCTION private.list_alterations_print_stations_impl()
RETURNS JSONB
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.create_alterations_print_station(
  p_name TEXT,
  p_priority INTEGER
)
RETURNS JSONB
...
SET search_path = ''
AS $function$
  SELECT private.create_alterations_print_station_impl(p_name, p_priority);
$function$;

CREATE OR REPLACE FUNCTION public.rotate_alterations_print_station_secret(
  p_station_id UUID
)
RETURNS JSONB
LANGUAGE SQL
...
SET search_path = ''
AS $function$
  SELECT private.rotate_alterations_print_station_secret_impl(p_station_id);
$function$;

CREATE OR REPLACE FUNCTION public.set_alterations_print_station_enabled(
  p_station_id UUID,
  p_enabled BOOLEAN
)
RETURNS JSONB
...
SET search_path = ''
AS $function$
  SELECT private.set_alterations_print_station_enabled_impl(
    p_station_id,
    p_enabled
  );
$function$;

CREATE OR REPLACE FUNCTION public.release_alterations_print_station_lease(
  p_station_id UUID
)
RETURNS JSONB
LANGUAGE SQL
...
SET search_path = ''
AS $function$
  SELECT private.release_alterations_print_station_lease_impl(p_station_id);
$function$;

CREATE OR REPLACE FUNCTION public.list_alterations_print_stations()
RETURNS JSONB
LANGUAGE SQL
STABLE
SECURITY INVOKER
SET search_path = ''
AS $function$
  SELECT private.list_alterations_print_stations_impl();
$function$;

REVOKE ALL ON FUNCTION private.create_alterations_print_station_impl(TEXT, INTEGER)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.rotate_alterations_print_station_secret_impl(UUID)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.set_alterations_print_station_enabled_impl(UUID, BOOLEAN)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.release_alterations_print_station_lease_impl(UUID)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.list_alterations_print_stations_impl()
  FROM PUBLIC, anon, authenticated;

REVOKE ALL ON FUNCTION public.create_alterations_print_station(TEXT, INTEGER)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.rotate_alterations_print_station_secret(UUID)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.set_alterations_print_station_enabled(UUID, BOOLEAN)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.release_alterations_print_station_lease(UUID)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.list_alterations_print_stations()
  FROM PUBLIC, anon, authenticated;

GRANT USAGE ON SCHEMA private TO authenticated;
GRANT EXECUTE ON FUNCTION private.create_alterations_print_station_impl(TEXT, INTEGER)
  TO authenticated;
GRANT EXECUTE ON FUNCTION private.rotate_alterations_print_station_secret_impl(UUID)
  TO authenticated;
GRANT EXECUTE ON FUNCTION private.set_alterations_print_station_enabled_impl(UUID, BOOLEAN)
  TO authenticated;
GRANT EXECUTE ON FUNCTION private.release_alterations_print_station_lease_impl(UUID)
  TO authenticated;
GRANT EXECUTE ON FUNCTION private.list_alterations_print_stations_impl()
  TO authenticated;

GRANT EXECUTE ON FUNCTION public.create_alterations_print_station(TEXT, INTEGER)
  TO authenticated;
GRANT EXECUTE ON FUNCTION public.rotate_alterations_print_station_secret(UUID)
  TO authenticated;
GRANT EXECUTE ON FUNCTION public.set_alterations_print_station_enabled(UUID, BOOLEAN)
  TO authenticated;
GRANT EXECUTE ON FUNCTION public.release_alterations_print_station_lease(UUID)
  TO authenticated;
GRANT EXECUTE ON FUNCTION public.list_alterations_print_stations()
  TO authenticated;
...
-- The workshop printer has no cash drawer, so this branch never accepts the
-- drawer job type or the drawer flag that the tailoring branch carries.
...
CREATE OR REPLACE FUNCTION private.retry_alterations_print_job_impl(
  p_job_id UUID
)
RETURNS JSONB
LANGUAGE plpgsql
...
BEGIN
  ...
  FROM public.print_jobs AS j
  WHERE j.id = p_job_id
    AND j.branch = 'alterations'
  FOR UPDATE;

  IF NOT FOUND THEN
    RAISE EXCEPTION USING
      ERRCODE = 'P0002',
      MESSAGE = 'alterations_print_job_not_found';
  END IF;
...
CREATE OR REPLACE FUNCTION private.cancel_alterations_print_job_impl(
  p_job_id UUID
)
RETURNS JSONB
LANGUAGE plpgsql
...
CREATE OR REPLACE FUNCTION public.enqueue_alterations_print_job(
  p_job_type TEXT,
  p_alteration_id UUID,
  p_payload JSONB,
  p_idempotency_key TEXT,
...
SET search_path = ''
AS $function$
  SELECT private.enqueue_alterations_print_job_impl(
    p_job_type,
    p_alteration_id,
    p_payload,
    p_idempotency_key,
...
CREATE OR REPLACE FUNCTION public.retry_alterations_print_job(p_job_id UUID)
RETURNS JSONB
LANGUAGE SQL
VOLATILE
SECURITY INVOKER
SET search_path = ''
AS $function$
  SELECT private.retry_alterations_print_job_impl(p_job_id);
$function$;

CREATE OR REPLACE FUNCTION public.cancel_alterations_print_job(p_job_id UUID)
RETURNS JSONB
LANGUAGE SQL
VOLATILE
SECURITY INVOKER
SET search_path = ''
AS $function$
  SELECT private.cancel_alterations_print_job_impl(p_job_id);
$function$;

REVOKE ALL ON FUNCTION private.enqueue_alterations_print_job_impl(
  TEXT, UUID, JSONB, TEXT, UUID
) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.retry_alterations_print_job_impl(UUID)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.cancel_alterations_print_job_impl(UUID)
  FROM PUBLIC, anon, authenticated;

REVOKE ALL ON FUNCTION public.enqueue_alterations_print_job(
  TEXT, UUID, JSONB, TEXT, UUID
) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.retry_alterations_print_job(UUID)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.cancel_alterations_print_job(UUID)
  FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION private.enqueue_alterations_print_job_impl(
  TEXT, UUID, JSONB, TEXT, UUID
) TO authenticated;
GRANT EXECUTE ON FUNCTION private.retry_alterations_print_job_impl(UUID)
  TO authenticated;
GRANT EXECUTE ON FUNCTION private.cancel_alterations_print_job_impl(UUID)
  TO authenticated;

GRANT EXECUTE ON FUNCTION public.enqueue_alterations_print_job(
  TEXT, UUID, JSONB, TEXT, UUID
) TO authenticated;
GRANT EXECUTE ON FUNCTION public.retry_alterations_print_job(UUID)
  TO authenticated;
GRANT EXECUTE ON FUNCTION public.cancel_alterations_print_job(UUID)
  TO authenticated;
...
CREATE OR REPLACE FUNCTION private.alterations_station_heartbeat_impl(
  p_station_id UUID,
  p_station_secret TEXT,
  p_app_version TEXT,
  p_printer_ip TEXT,
...
BEGIN
  v_station := private.require_alterations_print_station(
    p_station_id,
    p_station_secret
  );
  ...
  ELSIF v_lease.lease_expires_at <= v_now
    ...
    SELECT EXISTS (
      ...
      WHERE preferred.branch = 'alterations'
...
CREATE OR REPLACE FUNCTION private.alterations_station_claim_job_impl(
  p_station_id UUID,
  p_station_secret TEXT,
  p_generation BIGINT
)
...
BEGIN
  ...
  WHERE existing.branch = 'alterations'
  ...
  SET status = CASE
      ...
      job_lease_expires_at = NULL,
      updated_at = v_now
  WHERE expired.branch = 'alterations'
  ...
  WHERE queued.branch = 'alterations'
...
CREATE OR REPLACE FUNCTION private.alterations_station_begin_send_impl(
  p_station_id UUID,
  p_station_secret TEXT,
  p_generation BIGINT,
  p_job_id UUID,
...
CREATE OR REPLACE FUNCTION private.alterations_station_complete_job_impl(
  p_station_id UUID,
  p_station_secret TEXT,
  p_job_id UUID,
  p_job_token UUID
...
CREATE OR REPLACE FUNCTION private.alterations_station_fail_job_impl(
  p_station_id UUID,
  p_station_secret TEXT,
  p_job_id UUID,
  p_job_token UUID,
...
BEGIN
  ...
  ELSE
    v_status := 'pending';
    v_retry_at := v_now
      + private.alterations_retry_delay(v_job.attempt_count);
  END IF;
...
CREATE OR REPLACE FUNCTION private.alterations_station_release_impl(
  p_station_id UUID,
  p_station_secret TEXT,
  p_generation BIGINT
)
...
CREATE OR REPLACE FUNCTION public.alterations_station_heartbeat(
  p_station_id UUID,
  p_station_secret TEXT,
  p_app_version TEXT,
  p_printer_ip TEXT,
...
SET search_path = ''
AS $function$
  SELECT private.alterations_station_heartbeat_impl(
    p_station_id,
    p_station_secret,
    p_app_version,
    p_printer_ip,
...
CREATE OR REPLACE FUNCTION public.alterations_station_claim_job(
  p_station_id UUID,
  p_station_secret TEXT,
  p_generation BIGINT
)
...
SET search_path = ''
AS $function$
  SELECT private.alterations_station_claim_job_impl(
    p_station_id,
    p_station_secret,
    p_generation
  );
...
CREATE OR REPLACE FUNCTION public.alterations_station_begin_send(
  p_station_id UUID,
  p_station_secret TEXT,
  p_generation BIGINT,
  p_job_id UUID,
...
SET search_path = ''
AS $function$
  SELECT private.alterations_station_begin_send_impl(
    p_station_id,
    p_station_secret,
    p_generation,
    p_job_id,
...
CREATE OR REPLACE FUNCTION public.alterations_station_complete_job(
  p_station_id UUID,
  p_station_secret TEXT,
  p_job_id UUID,
  p_job_token UUID
...
SET search_path = ''
AS $function$
  SELECT private.alterations_station_complete_job_impl(
    p_station_id,
    p_station_secret,
    p_job_id,
    p_job_token
...
CREATE OR REPLACE FUNCTION public.alterations_station_fail_job(
  p_station_id UUID,
  p_station_secret TEXT,
  p_job_id UUID,
  p_job_token UUID,
...
SET search_path = ''
AS $function$
  SELECT private.alterations_station_fail_job_impl(
    p_station_id,
    p_station_secret,
    p_job_id,
    p_job_token,
...
CREATE OR REPLACE FUNCTION public.alterations_station_release(
  p_station_id UUID,
  p_station_secret TEXT,
  p_generation BIGINT
)
...
SET search_path = ''
AS $function$
  SELECT private.alterations_station_release_impl(
    p_station_id,
    p_station_secret,
    p_generation
  );
...
REVOKE ALL ON FUNCTION private.alterations_station_heartbeat_impl(
  UUID, TEXT, TEXT, TEXT, BOOLEAN, TEXT
) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.alterations_station_claim_job_impl(
  UUID, TEXT, BIGINT
) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.alterations_station_begin_send_impl(
  UUID, TEXT, BIGINT, UUID, UUID
) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.alterations_station_complete_job_impl(
  UUID, TEXT, UUID, UUID
) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.alterations_station_fail_job_impl(
  UUID, TEXT, UUID, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.alterations_station_release_impl(
  UUID, TEXT, BIGINT
) FROM PUBLIC, anon, authenticated;

REVOKE ALL ON FUNCTION public.alterations_station_heartbeat(
  UUID, TEXT, TEXT, TEXT, BOOLEAN, TEXT
) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.alterations_station_claim_job(
  UUID, TEXT, BIGINT
) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.alterations_station_begin_send(
  UUID, TEXT, BIGINT, UUID, UUID
) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.alterations_station_complete_job(
  UUID, TEXT, UUID, UUID
) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.alterations_station_fail_job(
  UUID, TEXT, UUID, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.alterations_station_release(
  UUID, TEXT, BIGINT
) FROM PUBLIC, anon, authenticated;

GRANT USAGE ON SCHEMA private TO anon;

GRANT EXECUTE ON FUNCTION private.alterations_station_heartbeat_impl(
  UUID, TEXT, TEXT, TEXT, BOOLEAN, TEXT
) TO anon;
GRANT EXECUTE ON FUNCTION private.alterations_station_claim_job_impl(
  UUID, TEXT, BIGINT
) TO anon;
GRANT EXECUTE ON FUNCTION private.alterations_station_begin_send_impl(
  UUID, TEXT, BIGINT, UUID, UUID
) TO anon;
GRANT EXECUTE ON FUNCTION private.alterations_station_complete_job_impl(
  UUID, TEXT, UUID, UUID
) TO anon;
GRANT EXECUTE ON FUNCTION private.alterations_station_fail_job_impl(
  UUID, TEXT, UUID, UUID, INTEGER, TEXT, TEXT
) TO anon;
GRANT EXECUTE ON FUNCTION private.alterations_station_release_impl(
  UUID, TEXT, BIGINT
) TO anon;

GRANT EXECUTE ON FUNCTION public.alterations_station_heartbeat(
  UUID, TEXT, TEXT, TEXT, BOOLEAN, TEXT
) TO anon;
GRANT EXECUTE ON FUNCTION public.alterations_station_claim_job(
  UUID, TEXT, BIGINT
) TO anon;
GRANT EXECUTE ON FUNCTION public.alterations_station_begin_send(
  UUID, TEXT, BIGINT, UUID, UUID
) TO anon;
GRANT EXECUTE ON FUNCTION public.alterations_station_complete_job(
  UUID, TEXT, UUID, UUID
) TO anon;
GRANT EXECUTE ON FUNCTION public.alterations_station_fail_job(
  UUID, TEXT, UUID, UUID, INTEGER, TEXT, TEXT
) TO anon;
GRANT EXECUTE ON FUNCTION public.alterations_station_release(
  UUID, TEXT, BIGINT
) TO anon;
