'use client'

import { useState, useEffect } from 'react'

// نوع البيانات للترجمات
type TranslationKey = string
type TranslationValue = string | { [key: string]: string }
type Translations = { [key: string]: TranslationValue }

// الترجمات العربية
const arTranslations: Translations = {
  'cutter': 'القصّاص',
  'select_cutter': 'اختيار القصّاص',
  'select_cutter_first': 'اختر القصّاص أولاً قبل تحديد العامل الخياط',
  'cutter_saved': 'تم حفظ القصّاص وتاريخ القص',
  'cut_date': 'تاريخ القص',
  'cut_orders': 'الطلبات التي تم قصّها',
  'no_cut_orders': 'لا توجد طلبات تم قصّها في الفترة المحددة',
  'workshop_managers': 'مدراء الورشة',
  'no_cutters': 'لا يوجد مدراء ورشة متاحون للاختيار',
  // أعمال الشك
  'shak_worker': 'شكّاك',
  'shak_workers': 'الشكّاكون',
  'shak_worker_dashboard': 'لوحة تحكم الشكّاك',
  'shak_orders': 'طلبات الشك',
  'shak_pending_orders': 'شك قيد التنفيذ',
  'shak_completed_orders': 'شك مكتمل',
  'complete_shak_work': 'إنهاء عمل الشك',
  'shak_done': 'انتهى عمل الشك',
  'no_pending_shak_orders': 'لا توجد طلبات شك قيد التنفيذ',
  'no_completed_shak_orders': 'لا توجد طلبات انتهى فيها عمل الشك',
  'view_pending_shak_orders': 'الطلبات التي فيها أعمال شك ولم تنتهِ بعد',
  'view_completed_shak_archive': 'سجل الطلبات التي انتهى فيها عمل الشك',
  // التنقل والعناوين الرئيسية
  'dashboard': 'لوحة التحكم',
  'orders': 'الطلبات',
  'appointments': 'المواعيد',
  'settings': 'الإعدادات',
  'workers': 'العمال',
  'reports': 'التقارير',
  'logout': 'تسجيل الخروج',
  'welcome': 'مرحباً',
  'welcome_back': 'مرحباً بعودتك',

  // الأزرار والإجراءات
  'add_new_order': 'إضافة طلب جديد',
  'book_appointment': 'حجز موعد',
  'view_details': 'عرض التفاصيل',
  'edit': 'تعديل',
  'delete': 'حذف',
  'save': 'حفظ',
  'cancel': 'إلغاء',
  'submit': 'إرسال',
  'search': 'بحث',
  'filter': 'تصفية',
  'export': 'تصدير',
  'print': 'طباعة',
  'print_order': 'طباعة الطلب',
  'back': 'رجوع',
  'next': 'التالي',
  'previous': 'السابق',
  'close': 'إغلاق',
  'confirm': 'تأكيد',
  'loading': 'جاري التحميل...',


  // حالات الطلبات
  'pending': 'في الانتظار',
  'in_progress': 'قيد التنفيذ',
  'completed': 'مكتمل',
  'delivered': 'تم التسليم',
  'cancelled': 'ملغي',

  // نصوص عامة
  'name': 'الاسم',
  'email': 'البريد الإلكتروني',
  'phone': 'رقم الهاتف',
  'address': 'العنوان',
  'date': 'التاريخ',
  'time': 'الوقت',
  'status': 'الحالة',
  'price': 'السعر',
  'paid_amount': 'الدفعة المستلمة',
  'remaining_amount': 'الدفعة المتبقية',
  'payment_warning': 'تنبيه دفعة متبقية',
  'remaining_payment_warning_message': 'يوجد دفعة متبقية غير مدفوعة',
  'payment_warning_question': 'هل تريد تحديث حالة الدفع قبل تسليم الطلب؟',
  'mark_as_paid': 'تم الدفع - تحديث المبلغ',
  'ignore_and_deliver': 'تجاهل وتسليم بصمت',
  'payment_warning_note': 'عند اختيار "تم الدفع"، سيتم تحديث المبلغ المدفوع ليساوي السعر الكلي وتغيير حالة الدفع إلى "مدفوع"',
  'note': 'ملاحظة',
  'total': 'المجموع',
  'description': 'الوصف',
  'notes': 'ملاحظات',
  'notes_section': 'قسم الملاحظات',
  'client_name': 'اسم الزبونة',
  'client_phone': 'رقم هاتف الزبونة',

  // رسائل النجاح والخطأ
  'success': 'نجح',
  'error': 'خطأ',
  'warning': 'تحذير',
  'info': 'معلومات',
  'order_added_success': 'تم حفظ الطلب بنجاح',
  'redirecting_to_orders': 'جاري التوجيه إلى صفحة الطلبات...',
  'order_updated_success': 'تم تحديث الطلب بنجاح',
  'order_deleted_success': 'تم حذف الطلب بنجاح',
  'order_update_error': 'حدث خطأ أثناء تحديث الطلب',
  'order_delete_error': 'حدث خطأ أثناء حذف الطلب',
  'order_add_error': 'حدث خطأ أثناء إضافة الطلب',
  'work_started_success': 'تم بدء العمل على الطلب',
  'work_start_error': 'حدث خطأ أثناء بدء العمل',
  'order_completed_success': 'تم إنهاء العمل على الطلب بنجاح',
  'order_complete_error': 'حدث خطأ أثناء إنهاء العمل',
  'fill_required_fields': 'يرجى ملء جميع الحقول المطلوبة',

  // نصوص إضافية مطلوبة
  'admin_dashboard': 'لوحة تحكم المدير',
  'worker_dashboard': 'لوحة تحكم العامل',
  'tailor_dashboard': 'لوحة تحكم الخياط',
  'admin': 'مدير',
  'worker': 'عامل',
  'tailor': 'خياط',
  'change_language': 'تغيير اللغة',
  'my_active_orders': 'طلباتي النشطة',
  'completed_orders': 'الطلبات المكتملة',
  'total_orders': 'إجمالي الطلبات',
  'total_revenue': 'إجمالي الإيرادات',
  'recent_orders': 'الطلبات الحديثة',
  'quick_actions': 'إجراءات سريعة',
  'view_all_orders': 'عرض جميع الطلبات',
  'add_order': 'إضافة طلب',
  'manage_workers': 'إدارة العمال',
  'view_reports': 'عرض التقارير',

  // مدير الورشة
  'workshop_manager': 'مدير الورشة',
  'workshop_manager_dashboard': 'لوحة تحكم مدير الورشة',
  'workshop_manager_description': 'إدارة ومتابعة الطلبات في الورشة',
  'workshop_manager_info': 'يمكنك متابعة جميع الطلبات في الورشة: الطلبات الحديثة، المكتملة، والمسلمة',
  'orders_management': 'إدارة الطلبات',
  'view_track_orders': 'عرض ومتابعة الطلبات الجارية',
  'manage_deliver_orders': 'إدارة الطلبات المنتهية وتسليمها',
  'view_delivered_history': 'عرض سجل الطلبات المسلمة',
  'delivered_orders': 'الطلبات المسلمة',
  'home_page': 'الصفحة الرئيسية',
  'client_name_required': 'اسم الزبونة *',
  'phone_required': 'رقم الهاتف *',
  'order_description_required': 'وصف الطلب *',
  'delivery_date_required': 'موعد التسليم *',
  'price_sar': 'السعر (ريال سعودي)',
  'measurements_cm': 'المقاسات (بالإنش)',
  'additional_notes': 'ملاحظات إضافية',
  'voice_notes_optional': 'ملاحظات صوتية (اختيارية)',
  'design_images': 'صور التصميم',
  'fabric_type': 'نوع القماش',
  'fabric_type_optional': 'نوع القماش',
  'responsible_worker': 'العامل المسؤول',
  'choose_worker': 'اختر العامل المسؤول',
  'status_and_worker': 'الحالة والعامل',
  'order_status': 'حالة الطلب',
  'additional_notes_placeholder': 'أي ملاحظات أو تفاصيل إضافية...',
  'order_description_placeholder': 'مثال: فستان سهرة طويل، فستان زفاف، عباية مطرزة...',
  'fabric_type_placeholder': 'مثال: حرير، شيفون، قطن، ساتان...',
  'save_order': 'حفظ الطلب',

  // حقول جديدة لصفحة إضافة الطلبات
  'order_received_date': 'تاريخ استلام الطلب',
  'payment_method': 'طريقة الدفع',
  'cash': 'كاش',
  'card': 'شبكة',
  'dress_description': 'وصف الفستان',
  'dress_description_placeholder': 'أدخل وصف تفصيلي للفستان...',
  'other_information': 'معلومات أخرى',
  'enter_client_name': 'أدخل اسم الزبونة',
  'enter_phone': 'أدخل رقم الهاتف',
  'enter_order_number': 'أدخل رقم الطلب',
  'optional': 'اختياري',
  'sar': 'ريال',
  'all_orders': 'جميع الطلبات',
  'reset_filters': 'إعادة تعيين الفلاتر',
  'filter_by_date': 'تصفية بالتاريخ',
  'search_completed_orders': 'البحث في الطلبات المكتملة...',
  'search_delivered_orders': 'البحث في الطلبات المسلمة...',
  'showing': 'عرض',
  'not_specified': 'غير محدد',
  'back_to_dashboard': 'العودة إلى لوحة التحكم',
  'overview_today': 'نظرة عامة على أنشطة اليوم',
  'welcome_worker': 'مرحباً بك في مساحة العمل',

  // مفاتيح مفقودة من لوحة التحكم
  'homepage': 'الصفحة الرئيسية',
  'my_completed_orders': 'طلباتي المكتملة',
  'my_total_orders': 'إجمالي طلباتي',
  'active_orders': 'الطلبات النشطة',
  'today_appointments': 'مواعيد اليوم',
  'statistics': 'الإحصائيات',
  'no_orders_found': 'لا توجد طلبات',
  'view_all': 'عرض الكل',
  'worker_management': 'إدارة العمال',
  'reminder': 'تذكير',
  'you_have': 'لديك',
  'today_appointments_reminder': 'موعد اليوم',
  'and': 'و',
  'orders_need_follow': 'طلبات تحتاج متابعة',
  'detailed_reports': 'تقارير مفصلة',
  'worker_description': 'يمكنك هنا متابعة طلباتك المخصصة لك وتحديث حالتها',

  // مفاتيح صفحة إضافة الطلبات

  'add_new_order_description': 'قم بإدخال تفاصيل الطلب الجديد بما في ذلك معلومات الزبونة والمقاسات',
  'basic_information': 'المعلومات الأساسية',
  'basic_measurements': 'المقاسات الأساسية',
  'advanced_measurements': 'المقاسات المتقدمة',
  'sleeve_measurements': 'مقاسات الأكمام',
  'length_measurements': 'مقاسات الطول',
  'cm_placeholder': 'إنش',
  'shoulder': 'الكتف',
  'shoulder_circumference': 'محيط الكتف',
  'chest': 'الصدر',
  'waist': 'الخصر',
  'hips': 'الأرداف',
  'dart_length': 'طول الخياطة',
  'bodice_length': 'طول الجسم',
  'neckline': 'خط الرقبة',
  'armpit': 'الإبط',
  'sleeve_length': 'طول الكم',
  'forearm': 'الساعد',
  'cuff': 'الكم',
  'front_length': 'الطول الأمامي',
  'back_length': 'الطول الخلفي',

  // مفاتيح صفحة الطلبات
  'order_number': 'رقم الطلب',
  'order_description': 'وصف الطلب',

  'search_placeholder': 'البحث بالاسم أو رقم الطلب أو الوصف...',
  'search_by_name': 'البحث بالاسم',
  'search_by_phone': 'البحث برقم الهاتف',
  'search_by_order_number': 'البحث برقم الطلب',

  'enter_phone_number': 'أدخل رقم الهاتف',

  'no_orders_assigned': 'لا توجد طلبات مخصصة لك',
  'no_orders_assigned_desc': 'لم يتم تخصيص أي طلبات لك بعد',
  'no_orders_found_desc': 'لا توجد طلبات مطابقة لمعايير البحث',
  'price_label': 'السعر',

  'view': 'عرض',
  'completing': 'جاري الإنهاء...',
  'start_work': 'بدء العمل',
  'complete_order': 'إنهاء الطلب',
  'complete_order_modal_title': 'إنهاء الطلب وتحميل صور العمل المكتمل',
  'important_warning': 'تحذير مهم',
  'complete_order_warning': 'بمجرد إنهاء الطلب، لن تتمكن من تعديل حالته مرة أخرى. تأكد من تحميل جميع صور العمل المكتمل قبل المتابعة.',
  'completed_work_images_label': 'صور العمل المكتمل',
  'click_to_upload_completed_work': 'اضغط لرفع صور العمل المكتمل (إلزامي)',
  'capture_photo': 'التقاط صورة',
  'use_camera_to_capture': 'استخدم الكاميرا لالتقاط صورة جديدة',
  'choose_from_gallery': 'اختيار من المعرض',
  'choose_existing_photos': 'اختر صور موجودة من جهازك',
  'work_image_label': 'صورة',
  'click_to_upload_more_images': 'اضغط لرفع المزيد من الصور',
  'must_upload_at_least_one': 'يجب رفع صورة واحدة على الأقل • التقط صورة أو اختر من المعرض',
  'take_photo_or_choose_gallery': 'التقط صورة أو اختر من المعرض أو اسحب الصور هنا',
  'important_warning_upload_required': 'تنبيه مهم - رفع الصور إلزامي',
  'must_upload_before_completing': 'يجب رفع صورة واحدة على الأقل للعمل المكتمل قبل إنهاء الطلب. الصور ضرورية لتوثيق جودة العمل.',
  'cannot_complete_without_images': 'لا يمكن إنهاء الطلب بدون رفع صور للعمل المكتمل',
  'order_deleted_successfully': 'تم حذف الطلب بنجاح',
  'select_delivery_date': 'اختر موعد التسليم',
  'busy_date_warning': 'تحذير: هذا التاريخ مزدحم بـ {count} طلبات. يمكنك الاستمرار لكن قد يتأخر التسليم.',
  'due_date': 'موعد التسليم',
  'proof_delivery_date': 'موعد البروفة',
  'delivered_date': 'تاريخ التسليم',
  'paid': 'المدفوع',

  // مفاتيح مكون حذف الطلب
  'confirm_delete_order': 'تأكيد حذف الطلب',
  'warning_delete_order': 'تحذير: حذف الطلب',
  'delete_order_warning_message': 'لا يمكن التراجع عن هذا الإجراء. سيتم حذف الطلب وجميع البيانات المرتبطة به نهائياً.',
  'admin_email': 'بريد المدير الإلكتروني',
  'admin_password': 'كلمة مرور المدير',
  'enter_admin_email': 'أدخل بريد المدير الإلكتروني',
  'enter_admin_password': 'أدخل كلمة مرور المدير',
  'please_fill_all_fields': 'يرجى ملء جميع الحقول',
  'email_does_not_match': 'البريد الإلكتروني لا يطابق بريد المدير المسجل',
  'incorrect_password': 'كلمة المرور غير صحيحة',
  'confirm_delete': 'تأكيد الحذف',

  // مفاتيح مكون الملاحظات الصوتية
  'start_recording': 'بدء التسجيل',
  'stop_recording': 'إيقاف التسجيل',
  'click_to_record_voice_note': 'انقر لتسجيل ملاحظة صوتية',
  'voice_notes': 'الملاحظات الصوتية',
  'voice_note': 'ملاحظة صوتية',
  'microphone_access_error': 'خطأ في الوصول للميكروفون',
  'transcribe_to_text': 'تحويل إلى نص',
  'transcribe_all': 'تحويل الكل',
  'no_notes_to_transcribe': 'لا توجد تسجيلات تحتاج للتحويل',
  'translate': 'ترجمة',
  'transcription': 'النص المحول',
  'translation': 'الترجمة',
  'select_language': 'اختر اللغة',
  'transcribing': 'جاري التحويل...',
  'translating': 'جاري الترجمة...',
  'transcription_error': 'خطأ في تحويل الصوت إلى نص',
  'translation_error': 'خطأ في الترجمة',
  'english': 'الإنجليزية',
  'urdu': 'الأوردو',
  'bengali': 'البنغالية',
  'edit_transcription': 'تعديل النص',

  // مفاتيح صفحة العمال
  'worker_added_success': 'تم إضافة العامل بنجاح',
  'error_adding_worker': 'حدث خطأ أثناء إضافة العامل',
  'worker_updated_success': 'تم تحديث العامل بنجاح',
  'error_updating_worker': 'حدث خطأ أثناء تحديث العامل',
  'worker_deleted_success': 'تم حذف العامل بنجاح',
  'worker_deactivated': 'تم إلغاء تفعيل العامل',
  'worker_activated': 'تم تفعيل العامل',
  'adding': 'جاري الإضافة...',
  'add_worker': 'إضافة عامل',
  'active': 'نشط',
  'inactive': 'غير نشط',
  'save_changes': 'حفظ التغييرات',
  'search_workers_placeholder': 'البحث عن العمال...',
  'workers_management': 'إدارة العمال',
  'view_manage_team': 'عرض وإدارة فريق العمل في ورشة التفصيل',
  'add_new_worker': 'إضافة عامل جديد',
  'add_new_worker_form': 'إضافة عامل جديد',
  'full_name_required': 'الاسم الكامل *',
  'email_required': 'البريد الإلكتروني *',
  'password_required': 'كلمة المرور *',
  'phone_required_worker': 'رقم الهاتف *',
  'enter_full_name': 'أدخل الاسم الكامل',
  'enter_email': 'أدخل البريد الإلكتروني',
  'enter_password': 'أدخل كلمة المرور',
  'specialty_required': 'التخصص *',
  'specialty_example': 'مثال: خياطة فساتين السهرة',
  'edit_worker': 'تعديل العامل',
  'new_password': 'كلمة المرور الجديدة',
  'leave_empty_no_change': 'اتركه فارغاً إذا لم ترد تغييره',
  'no_workers': 'لا يوجد عمال',
  'no_workers_found': 'لا يوجد عمال مطابقين لمعايير البحث',
  'joined_on': 'انضم في',
  'total_workers': 'إجمالي العمال',
  'active_workers': 'العمال النشطون',
  'total_completed_orders': 'إجمالي الطلبات المكتملة',
  'confirm_delete_worker': 'هل أنت متأكد من حذف هذا العامل؟',

  // مفاتيح صفحة التقارير
  'engagement_dress': 'فستان خطوبة',
  'casual_dress': 'فستان يومي',
  'other': 'أخرى',
  'this_week': 'هذا الأسبوع',
  'this_month': 'هذا الشهر',
  'this_quarter': 'هذا الربع',
  'this_year': 'هذا العام',
  'reports_analytics': 'التقارير والتحليلات',
  'comprehensive_analysis': 'تحليل شامل لأداء الأعمال',
  'monthly_trend': 'الاتجاه الشهري',
  'revenue_label': 'الإيرادات',

  // مفاتيح صفحة المواعيد
  'confirmed': 'مؤكد',
  'pm': 'مساءً',
  'am': 'صباحاً',
  'all_statuses': 'جميع الحالات',
  'all_dates': 'جميع التواريخ',
  'today': 'اليوم',
  'tomorrow': 'غداً',
  'appointments_management': 'إدارة المواعيد',
  'view_manage_appointments': 'عرض وإدارة جميع مواعيد التفصيل',
  'book_new_appointment': 'حجز موعد جديد',
  'search_appointments_placeholder': 'البحث في المواعيد...',
  'no_appointments': 'لا توجد مواعيد',
  'no_appointments_found': 'لا توجد مواعيد مطابقة لمعايير البحث',
  'created_on': 'تم الإنشاء في',
  'confirm_appointment': 'تأكيد الموعد',
  'cancel_appointment': 'إلغاء الموعد',
  'total_appointments': 'إجمالي المواعيد',
  'edit_appointment': 'تعديل الموعد',
  'confirm_delete_appointment_message': 'هل أنت متأكد من حذف هذا الموعد؟ لا يمكن التراجع عن هذا الإجراء.',
  'yes_delete': 'نعم، احذف',
  'deleting': 'جاري الحذف...',

  'email_placeholder': 'مثال: example@email.com',
  'customer_name_placeholder': 'مثال: أحمد محمد',

  'notes_placeholder': 'أي ملاحظات إضافية...',
  'service_type': 'نوع الخدمة',
  'customer_name': 'اسم العميل',
  'date_time': 'التاريخ والوقت',

  // مفاتيح مكونات إضافية
  'of': 'من',
  'images_text': 'صور',

  // مفاتيح مكونات الصور والتحميل
  'max_images_reached': 'تم الوصول للحد الأقصى من الصور',
  'drop_images_here': 'اسقط الصور هنا',
  'click_or_drag_images': 'انقر أو اسحب الصور هنا',
  'image_upload_format': 'PNG, JPG, JPEG حتى 5MB',
  'max_images_text': 'الحد الأقصى',
  'order_label': 'الطلب',
  'for_client': 'للزبونة',

  // مفاتيح مكون تعديل الطلب
  'edit_order': 'تعديل الطلب',
  'price_sar_required': 'السعر (ريال سعودي) *',
  'status_pending': 'في الانتظار',
  'status_in_progress': 'قيد التنفيذ',

  'status_delivered': 'تم التسليم',
  'status_cancelled': 'ملغي',

  // نصوص الفوتر
  'home': 'الرئيسية',
  'track_order': 'استعلام عن الطلب',
  'fabrics': 'الأقمشة',
  'contact_us': 'تواصلي معنا',
  'yasmin_alsham': 'ياسمين الشام',
  'custom_dress_tailoring': 'تفصيل فساتين حسب الطلب',

  // مفاتيح مكون تفاصيل الطلب (OrderModal)
  'order_details': 'تفاصيل الطلب',
  'alterations_section_title': 'التعديلات',
  'first_proof_alteration': 'تعديل البروفة الأولى',
  'second_proof_alteration': 'تعديل البروفة الثانية',
  'post_delivery_alteration': 'تعديل بعد التسليم',
  'post_delivery_alterations_count': 'تعديلات بعد التسليم',
  'order_without_alteration': 'طلب بدون تعديل',
  'translate_alteration_text': 'ترجمة النص للهندية',
  'restore_alteration_text': 'استعادة النسخة الافتراضية',
  'loading_alterations': 'جاري تحميل التعديلات...',
  'alterations_load_error': 'تعذر تحميل التعديلات المرتبطة بهذا الطلب.',
  'alteration_translation_error': 'تعذرت ترجمة نص التعديل. حاول مرة أخرى.',
  'alteration_translation_save_error': 'تمت الترجمة، لكن تعذر حفظها. حاول مرة أخرى لحفظ النسخة نفسها.',
  'alteration_text_not_available': 'لا يوجد نص مكتوب لهذا التعديل.',
  'alteration_photos_label': 'صور التعديل',
  'alteration_photo_alt': 'صورة التعديل {number}',
  'open_alteration_photo': 'عرض صورة التعديل {number} بالحجم الكامل',
  'alteration_status_pending': 'قيد الانتظار',
  'alteration_status_in_progress': 'قيد التنفيذ',
  'alteration_status_completed': 'مكتمل',
  'alteration_status_delivered': 'تم التسليم',
  'alteration_status_cancelled': 'ملغي',
  'alteration_error_reason': 'سبب الخطأ',
  'alteration_error_tailor_error': 'خطأ الخياط',
  'alteration_error_cutter_error': 'خطأ القصاص',
  'alteration_error_measurement_error': 'خطأ قياس',
  'alteration_error_customer_error': 'خطأ زبونة',
  'alteration_error_reception_error': 'خطأ الاستقبال',
  'alteration_error_other_error': 'خطأ آخر',
  'customer_information': 'معلومات الزبونة',
  'order_date': 'تاريخ الطلب',

  'assigned_worker': 'العامل',
  'chest_bust': 'الصدر',
  'advanced_tailoring_measurements': 'مقاسات التفصيل المتقدمة',
  'additional_measurements': 'مقاسات إضافية',
  'dress_length': 'طول الفستان',
  'shoulder_width': 'عرض الكتف',
  'sleeve_length_old': 'طول الكم',
  'design_image_alt': 'صورة التصميم',
  'design_links': 'روابط التصميم',
  'design_links_label': 'روابط التصاميم (اختياري)',
  'design_links_placeholder': 'أضف روابط التصاميم المرجعية هنا، كل رابط في سطر منفصل...',
  'completed_work_images': 'صور العمل المكتمل',
  'completed_work_description': 'تم رفع صور العمل المكتمل من قبل العامل',
  'completed_work_image_alt': 'صورة العمل المكتمل',

  // مفاتيح صفحة الطلبات
  'view_manage_orders': 'عرض وإدارة جميع الطلبات',
  'order_date_label': 'تاريخ الطلب',
  'delivery_date_label': 'موعد التسليم',
  'worker_label': 'العامل',
  'fabric_label': 'القماش:',
  'notes_label': 'ملاحظات',

  // مفاتيح صفحة العمال
  'experience_years': 'سنوات الخبرة',

  // مفاتيح إضافية للوحة التحكم
  'control_panel': 'لوحة التحكم',
  'completed_orders_management': 'الطلبات المكتملة',
  'delivered_orders_management': 'الطلبات المسلمة',
  'ready_designs_management': 'إدارة التصاميم الجاهزة',
  'fabrics_management': 'إدارة الأقمشة',
  'orders_in_progress': 'طلبات قيد التنفيذ',
  'appointments_today': 'مواعيد اليوم',
  'finished_orders': 'طلبات منتهية',
  'new_orders': 'طلبات جديدة',
  'delivered_orders_count': 'طلبات مسلمة',
  'view_all_orders_button': 'جميع الطلبات',

  'productive_day_message': 'نتمنى لك يوم عمل مثمر في تنفيذ الطلبات',

  // مفاتيح صفحة طلباتي المكتملة للعامل
  'worker_completed_orders_desc': 'الطلبات التي أنهيت العمل عليها',
  'search_by_name_or_order': 'ابحث باسم العميل أو رقم الطلب...',
  'no_completed_orders': 'لا توجد طلبات مكتملة',
  'no_completed_orders_desc': 'لم تكمل أي طلبات بعد. ستظهر الطلبات هنا بعد إنهائها.',
  'photos': 'صور',
  'order_info': 'معلومات الطلب',

  'measurements': 'المقاسات',
  'design': 'تصميم',
  'enlarged_image': 'صورة مكبرة',

  // مفاتيح المقاسات الجديدة
  'add_measurements': 'إضافة المقاسات',
  'edit_measurements': 'تعديل المقاسات',
  'measurements_modal_title': 'المقاسات',
  'save_measurements': 'حفظ المقاسات',
  'measurement_sh': 'الكتف (sh)',
  'measurement_shr': 'دوران الكتف (shr)',
  'measurement_ch': 'الصدر (ch)',
  'measurement_w': 'الخصر (w)',
  'measurement_hi': 'الأرداف (hi)',
  'measurement_p': 'طول البنس (p)',
  'measurement_L': 'طول الصدرية (L)',
  'measurement_v': 'فتحة الصدر (v)',
  'measurement_HF': 'الإبط (HF)',
  'measurement_K': 'طول الكم (K)',
  'measurement_S': 'الزند (S)',
  'measurement_S1': 'الإسوارة (S1)',
  'measurement_L_front': 'طول الأمام (L)',
  'measurement_LB': 'طول الخلف (LB)',
  'measurement_additional_notes': 'مقاسات إضافية',
  'additional_measurements_placeholder': 'أدخل أي مقاسات إضافية هنا...',
  'measurements_saved_successfully': 'تم حفظ المقاسات بنجاح',
  'measurements_save_error': 'حدث خطأ أثناء حفظ المقاسات',

  // مفاتيح لوحة تحكم الخياط
  'tailor_dashboard_desc': 'إدارة ومتابعة طلبات الخياطة الخاصة بك',
  'tailor_workspace': 'مساحة عمل الخياط',
  'tailor_workspace_info': 'يمكنك متابعة طلباتك الحالية، وتحديث حالاتها، وعرض أرشيف الطلبات المكتملة',
  'worker_attendance_title': 'الحضور والانصراف',
  'worker_attendance_source': 'مسترد من سجل الحضور والانصراف · توقيت الرياض',
  'worker_attendance_load_error': 'تعذر تحميل ملخص الحضور',
  'morning_arrival_time': 'ساعة الحضور صباحاً',
  'not_checked_in_today': 'لم يُسجّل بعد',
  'current_month_absence': 'الغياب في الشهر الحالي',
  'friday_excluded_note': 'الجمعة غير محسوبة ضمن الغياب',
  'today_label': 'اليوم',
  'days_unit': 'أيام',
  'refresh': 'تحديث',
  'current_orders': 'الطلبات الجارية',
  'view_update_orders': 'عرض وتحديث حالة الطلبات المسندة إليك',
  'view_completed_archive': 'عرض سجل الطلبات التي أنجزتها',

  // مفاتيح صفحة الطلبات المكتملة
  'completed_orders_subtitle': 'الطلبات التي أنهيتها وجاهزة للتسليم',
  'total_completed_orders_label': 'إجمالي الطلبات المكتملة:',
  'no_completed_orders_title': 'لا توجد طلبات مكتملة',
  'no_completed_orders_message': 'لا توجد طلبات مكتملة في الوقت الحالي.',
  'delivery_success_message': 'تم تحديث حالة الطلب إلى "تم التسليم" بنجاح',
  'request_alteration': 'طلب تعديل',
  'delete_order': 'حذف الطلب',
  'all_months': 'كل الأشهر',
  'current_month_label': 'الشهر الحالي',
  'month_orders_label': 'طلب',
  'completed_at_label': 'أُنهي بتاريخ',
  'worker_completed_at_label': 'تاريخ إنهاء العمل',
  'month_filter_label': 'فلتر الشهر',
  'orders_of_month': 'طلبات',

  // Salary Dashboard Translations - Arabic
  'salary_dashboard_title': 'رواتب قسم التفصيل',
  'salary_dashboard_desc': 'الراتب الثابت والعمل بالقطعة في لوحة شهرية واحدة',
  'payroll_type_label': 'نظام الراتب',
  'base_salary_label': 'الراتب الثابت',
  'overtime_hours_label': 'ساعات إضافية',
  'overtime_value_label': 'قيمة الإضافي',
  'piece_work_details_label': 'تفاصيل العمل بالقطعة',
  'small_advances_label': 'سلف صغيرة',
  'large_debt_label': 'ديون كبيرة / سداد',
  'net_salary_label': 'الصافي',
  'actions_label': 'إجراءات',
  'fixed_salary_option': 'راتب ثابت',
  'piece_rate_option': 'بالقطعة',
  'view_items_button': 'عرض القطع',
  'repay_button': 'سداد',
  'settle_salary_button': 'تصفية الراتب',
  'total_net_salary': 'إجمالي الرواتب الصافية (للعمال الظاهرين)',
  'add_transaction_title': 'إضافة سلفة أو دين',
  'transaction_type_label': 'النوع',
  'small_advance_option': 'سلفة صغيرة (تخصم هذا الشهر)',
  'large_debt_option': 'دين كبير (يبقى في الذمة)',
  'amount_label': 'المبلغ',
  'date_label': 'التاريخ',
  'note_label': 'ملاحظة (اختياري)',
  'add_transaction_button': 'إضافة العملية',
  'saving': 'جاري الحفظ...',
  'no_workers_found_salary': 'لا يوجد عمال',
  'loading_salary_dashboard': 'جاري تحميل لوحة الرواتب...',
  'salary_settlement_success': 'تم حفظ تصفية الراتب بنجاح.',
  'salary_settlement_error': 'فشل حفظ تصفية الراتب.',
  'transaction_success': 'تم حفظ العملية بنجاح.',
  'transaction_error': 'فشل حفظ العملية.',
  'select_worker_alert': 'الرجاء اختيار عامل أولاً.',
  'invalid_amount_alert': 'الرجاء إدخال مبلغ صحيح أكبر من صفر.',
  'negative_net_salary_alert': 'الراتب الصافي سالب. يرجى تعديل المدخلات قبل الحفظ.',
  'piece_rate_items_modal_title': 'قطع العمل المنجزة',
  'close_button': 'إغلاق',
  'item_title': 'عنوان القطعة',
  'price_sar_label': 'السعر (ر.س)',
  'no_items_for_month': 'لا توجد قطع منجزة لهذا الشهر.',

  'switch_to_piece_rate': 'التحويل لنظام القطعة',
  'switch_to_fixed_salary': 'التحويل للراتب الثابت',
  'has_debt_outstanding': 'يوجد دين: متبقي',
  'debt_repayment_amount_label': 'مبلغ سداد الدين',
  'total_advances_auto_label': 'إجمالي السلف (تلقائي)',
  'total_large_debt_label': 'إجمالي الديون المرحلة',
  'debt_repayment_manual_label': 'سداد الدين (يدوي)',
  'remaining_net_salary_label': 'صافي الراتب المتبقي',
  'final_net_salary_label': 'الراتب الصافي النهائي',
  'total_item_income_label': 'إجمالي دخل القطع',
  'completed_items_label': 'القطع / الفساتين المنجزة',
  'loading_items': 'جاري تحميل القطع...',
  'price_wage_placeholder': 'السعر / الأجرة',
  'save_monthly_salary_button': 'حفظ الراتب الشهري',

  // مفاتيح قسم تعليقات التصميم في OrderModal
  'view_front': 'أمام',
  'view_back': 'خلف',
  'design_comments': 'تعليقات على التصميم',
  'comment_badge': '{count} تعليق',
  'comment_default_title': 'التعليق',
  'markers_and_drawings': '({markers} علامة، {drawings} رسم)',
  'annotations_count': 'التعليقات ({count})',
  'stop_audio': 'إيقاف',
  'play_audio': 'تشغيل الصوت',
  'translation_label': 'الترجمة',
  'marker_without_comment': 'علامة بدون تعليق',

  'add': 'إضافة',
  'primary_fabric': 'القماش الرئيسي',
  'select_fabric_first': 'يرجى اختيار صورة القماش أولاً أو إضافة قماش ثانوي',
  'ai_notes_placeholder': 'أي تعليمات أو ملاحظات إضافية للذكاء الاصطناعي (مثال: أريد الفستان بأكمام طويلة، أضف ترتر على الصدر...)',

  // مفاتيح توليد التصميم بالذكاء الاصطناعي (GenerateDesignButton)
  'ai_generate_design': 'توليد التصميم بالذكاء الاصطناعي',
  'generated_designs_count': 'التصاميم المولدة ({count})',
  'all_generated_designs': 'جميع التصاميم المولدة ({count})',
  'select_fabric_image': 'اختر صورة القماش',
  'fabric_image_desc': 'اختر صورة أو أكثر من صور الطلب لاستخدامها كمرجع للقماش',
  'no_images_available': '(لا توجد صور متاحة)',
  'add_secondary_fabric': 'إضافة قماش ثانوي',
  'generate_design': 'توليد التصميم',
  'select_fabric_location': 'تحديد مكان كل قماش في الجسم',
  'fabric_location_desc': 'يمكنك اختيار أكثر من منطقة لكل قماش',
  'other_location': 'مكان آخر',
  'type_location_placeholder': 'اكتب المكان...',
  'skip': 'تخطي',
  'generating_design_heading': 'جاري توليد التصميم',
  'analyzing_design': 'جاري تحليل التصميم وإنشاء الوصف الاحترافي...',
  'generating_image_text': 'جاري توليد صورة التصميم...',
  'generation_step': 'الخطوة {step} من 2 — يمكنك حفظ الطلب الآن والانتظار',
  'design_generated_success': 'تم توليد التصميم بنجاح!',
  'generate_new_design': 'توليد تصميم جديد',
  'other_fabric': 'قماش آخر',
  'type_fabric_placeholder': 'اكتب نوع القماش...',
  'select_action': 'اختيار',
  'fabric_color': 'لون القماش',
  'other_color': 'لون آخر',
  'type_color_placeholder': 'اكتب اللون...',
  'download': 'تحميل',
  'design_number': 'تصميم {n}',
  'generated_design_alt': 'تصميم مولد {n}',
  'image_alt_number': 'صورة {n}',

  // مفاتيح قسم التصميم الكرتوني في OrderModal
  'converting_to_cartoon': 'جاري التحويل...',
  'convert_to_cartoon': 'تحويل الصورة الى كرتون',
  'cartoon_design': 'التصميم الكرتوني',
  'print_image': 'طباعة الصورة',
  'regenerate': 'إعادة التوليد',
  'no_completed_image_title': 'لا توجد صورة عمل مكتمل',
  'no_completed_image_desc': 'لم يتم رفع صورة للعمل المكتمل بعد. يمكنك رفع صورة من جهازك لاستخدامها في التحويل إلى كرتون.',
  'uploaded_image_alt': 'الصورة المرفوعة',
  'convert_to_cartoon_btn': 'تحويل إلى كرتون',
  'change': 'تغيير',
  'click_to_upload_dress': 'اضغط لرفع صورة الفستان',

  // تقييم الإدارة في صفحة الطلبات المكتملة للعامل
  'management_rating_title': 'تقييم الإدارة',
  'piece_wage_label': 'أجر القطعة',
  'sar_unit': 'ر.س',
  'bonus_label': 'مكافأة',
  'rating_out_of': '{rating}/5',
  'visible_salary_total': 'إجمالي الراتب',
  'visible_salary_note': 'التسعير الظاهر من الإدارة',
  'completed_work_thumbnail_alt': 'صورة العمل المكتمل للطلب {order}',
  'no_completed_work_image': 'لا توجد صورة للعمل المكتمل',

  'language': 'ar'
}

// الترجمات الإنجليزية
const enTranslations: Translations = {
  'cutter': 'Cutter',
  'select_cutter': 'Select cutter',
  'select_cutter_first': 'Select the cutter before assigning the tailor',
  'cutter_saved': 'Cutter and cutting date saved',
  'cut_date': 'Cutting date',
  'cut_orders': 'Cut orders',
  'no_cut_orders': 'No cut orders in the selected period',
  'workshop_managers': 'Workshop managers',
  'no_cutters': 'No workshop managers available for assignment',
  // Shak (basting) work
  'shak_worker': 'Shak worker',
  'shak_workers': 'Shak workers',
  'shak_worker_dashboard': 'Shak worker dashboard',
  'shak_orders': 'Shak orders',
  'shak_pending_orders': 'Shak in progress',
  'shak_completed_orders': 'Shak completed',
  'complete_shak_work': 'Complete shak work',
  'shak_done': 'Shak work done',
  'no_pending_shak_orders': 'No shak work in progress',
  'no_completed_shak_orders': 'No orders with completed shak work',
  'view_pending_shak_orders': 'Orders with shak work that is not finished yet',
  'view_completed_shak_archive': 'Archive of orders whose shak work is finished',
  // التنقل والعناوين الرئيسية
  'dashboard': 'Dashboard',
  'orders': 'Orders',
  'appointments': 'Appointments',
  'settings': 'Settings',
  'workers': 'Workers',
  'reports': 'Reports',
  'logout': 'Logout',
  'welcome': 'Welcome',
  'welcome_back': 'Welcome Back',

  // الأزرار والإجراءات
  'add_new_order': 'Add New Order',
  'book_appointment': 'Book Appointment',
  'view_details': 'View Details',
  'edit': 'Edit',
  'delete': 'Delete',
  'save': 'Save',
  'cancel': 'Cancel',
  'submit': 'Submit',
  'search': 'Search',
  'filter': 'Filter',
  'export': 'Export',
  'print': 'Print',
  'print_order': 'Print Order',
  'back': 'Back',
  'next': 'Next',
  'previous': 'Previous',
  'close': 'Close',
  'confirm': 'Confirm',
  'loading': 'Loading...',


  // حالات الطلبات
  'pending': 'Pending',
  'in_progress': 'In Progress',
  'completed': 'Completed',
  'delivered': 'Delivered',
  'cancelled': 'Cancelled',

  // نصوص عامة
  'name': 'Name',
  'email': 'Email',
  'phone': 'Phone',
  'address': 'Address',
  'date': 'Date',
  'time': 'Time',
  'status': 'Status',
  'price': 'Price',
  'paid_amount': 'Paid Amount',
  'remaining_amount': 'Remaining Amount',
  'payment_warning': 'Payment Warning',
  'remaining_payment_warning_message': 'There is an unpaid remaining amount',
  'payment_warning_question': 'Do you want to update the payment status before delivering the order?',
  'mark_as_paid': 'Mark as Paid - Update Amount',
  'ignore_and_deliver': 'Ignore and Deliver Silently',
  'payment_warning_note': 'When selecting "Mark as Paid", the paid amount will be updated to equal the total price and the payment status will be changed to "Paid"',
  'note': 'Note',
  'total': 'Total',
  'description': 'Description',
  'notes': 'Notes',
  'notes_section': 'Notes Section',
  'client_name': 'Client Name',
  'client_phone': 'Client Phone',

  // رسائل النجاح والخطأ
  'success': 'Success',
  'error': 'Error',
  'warning': 'Warning',
  'info': 'Info',
  'order_added_success': 'Order saved successfully',
  'redirecting_to_orders': 'Redirecting to orders page...',
  'order_updated_success': 'Order updated successfully',
  'order_deleted_success': 'Order deleted successfully',
  'order_update_error': 'Error updating order',
  'order_delete_error': 'Error deleting order',
  'order_add_error': 'Error adding order',
  'work_started_success': 'Work started on order',
  'work_start_error': 'Error starting work',
  'order_completed_success': 'Order completed successfully',
  'order_complete_error': 'Error completing order',
  'fill_required_fields': 'Please fill all required fields',

  // نصوص إضافية مطلوبة
  'admin_dashboard': 'Admin Dashboard',
  'worker_dashboard': 'Worker Dashboard',
  'tailor_dashboard': 'Tailor Dashboard',
  'admin': 'Admin',
  'worker': 'Worker',
  'tailor': 'Tailor',
  'change_language': 'Change Language',
  'my_active_orders': 'My Active Orders',
  'completed_orders': 'Completed Orders',
  'total_orders': 'Total Orders',
  'total_revenue': 'Total Revenue',
  'recent_orders': 'Recent Orders',
  'quick_actions': 'Quick Actions',
  'view_all_orders': 'View All Orders',
  'add_order': 'Add Order',
  'manage_workers': 'Manage Workers',
  'view_reports': 'View Reports',

  // Workshop Manager
  'workshop_manager': 'Workshop Manager',
  'workshop_manager_dashboard': 'Workshop Manager Dashboard',
  'workshop_manager_description': 'Manage and track workshop orders',
  'workshop_manager_info': 'You can track all workshop orders: recent, completed, and delivered orders',
  'orders_management': 'Orders Management',
  'view_track_orders': 'View and track ongoing orders',
  'manage_deliver_orders': 'Manage and deliver completed orders',
  'view_delivered_history': 'View delivered orders history',
  'delivered_orders': 'Delivered Orders',
  'home_page': 'Home Page',
  'client_name_required': 'Client Name *',
  'phone_required': 'Phone Number *',
  'order_description_required': 'Order Description *',
  'delivery_date_required': 'Delivery Date *',
  'price_sar': 'Price (SAR)',
  'measurements_cm': 'Measurements (inch)',
  'additional_notes': 'Additional Notes',
  'voice_notes_optional': 'Voice Notes (Optional)',
  'design_images': 'Design Images',
  'fabric_type': 'Fabric Type',
  'fabric_type_optional': 'Fabric Type',
  'responsible_worker': 'Responsible Worker',
  'choose_worker': 'Choose Responsible Worker',
  'status_and_worker': 'Status and Worker',
  'order_status': 'Order Status',
  'additional_notes_placeholder': 'Any additional notes or details...',
  'order_description_placeholder': 'Example: Long evening dress, wedding dress, embroidered abaya...',
  'fabric_type_placeholder': 'Example: Silk, chiffon, cotton, satin...',
  'save_order': 'Save Order',

  // New fields for add order page
  'order_received_date': 'Order Received Date',
  'payment_method': 'Payment Method',
  'cash': 'Cash',
  'card': 'Card/Network',
  'dress_description': 'Dress Description',
  'dress_description_placeholder': 'Enter detailed dress description...',
  'other_information': 'Other Information',

  'enter_phone': 'Enter phone number',
  'enter_order_number': 'Enter order number',
  'optional': 'Optional',


  'reset_filters': 'Reset Filters',
  'filter_by_date': 'Filter by Date',
  'search_completed_orders': 'Search completed orders...',
  'search_delivered_orders': 'Search delivered orders...',
  'showing': 'Showing',
  'not_specified': 'Not Specified',
  'back_to_dashboard': 'Back to Dashboard',
  'overview_today': 'Overview of today\'s activities',
  'welcome_worker': 'Welcome to your workspace',

  // مفاتيح مفقودة من لوحة التحكم
  'homepage': 'Homepage',
  'my_completed_orders': 'My Completed Orders',
  'my_total_orders': 'My Total Orders',
  'active_orders': 'Active Orders',
  'today_appointments': 'Today\'s Appointments',
  'statistics': 'Statistics',
  'no_orders_found': 'No orders found',
  'view_all': 'View All',
  'worker_management': 'Worker Management',
  'reminder': 'Reminder',
  'you_have': 'You have',
  'today_appointments_reminder': 'appointments today',
  'and': 'and',
  'orders_need_follow': 'orders that need follow-up',
  'detailed_reports': 'Detailed Reports',
  'worker_description': 'Here you can track your assigned orders and update their status',

  // مفاتيح صفحة إضافة الطلبات

  'add_new_order_description': 'Enter the new order details including client information and measurements',
  'basic_information': 'Basic Information',
  'basic_measurements': 'Basic Measurements',
  'advanced_measurements': 'Advanced Measurements',
  'sleeve_measurements': 'Sleeve Measurements',
  'length_measurements': 'Length Measurements',
  'cm_placeholder': 'inch',
  'shoulder': 'Shoulder',
  'shoulder_circumference': 'Shoulder Circumference',
  'chest': 'Chest',
  'waist': 'Waist',
  'hips': 'Hips',
  'dart_length': 'Dart Length',
  'bodice_length': 'Bodice Length',
  'neckline': 'Neckline',
  'armpit': 'Armpit',
  'sleeve_length': 'Sleeve Length',
  'forearm': 'Forearm',
  'cuff': 'Cuff',
  'front_length': 'Front Length',
  'back_length': 'Back Length',

  // مفاتيح صفحة الطلبات
  'order_number': 'Order Number',
  'order_description': 'Order Description',

  'search_placeholder': 'Search by name, order number, or description...',
  'search_by_name': 'Search by Name',
  'search_by_phone': 'Search by Phone',
  'search_by_order_number': 'Search by Order Number',

  'enter_phone_number': 'Enter phone number',

  'no_orders_assigned': 'No orders assigned to you',
  'no_orders_assigned_desc': 'No orders have been assigned to you yet',
  'no_orders_found_desc': 'No orders found matching the search criteria',
  'price_label': 'Price',

  'view': 'View',
  'completing': 'Completing...',
  'start_work': 'Start Work',
  'complete_order': 'Complete Order',
  'complete_order_modal_title': 'Complete Order and Upload Finished Work Images',
  'important_warning': 'Important Warning',
  'complete_order_warning': 'Once you complete the order, you will not be able to modify its status again. Make sure to upload all finished work images before proceeding.',
  'completed_work_images_label': 'Finished Work Images',
  'click_to_upload_completed_work': 'Click to upload finished work images (required)',
  'capture_photo': 'Take a Photo',
  'use_camera_to_capture': 'Use the camera to take a new photo',
  'choose_from_gallery': 'Choose from Gallery',
  'choose_existing_photos': 'Select existing photos from your device',
  'work_image_label': 'Image',
  'click_to_upload_more_images': 'Click to upload more images',
  'must_upload_at_least_one': 'At least one image is required • Take a photo or choose from gallery',
  'take_photo_or_choose_gallery': 'Take a photo, choose from gallery, or drag images here',
  'important_warning_upload_required': 'Important Notice - Image Upload Required',
  'must_upload_before_completing': 'At least one finished work image must be uploaded before completing the order. Images are required to document work quality.',
  'cannot_complete_without_images': 'Cannot complete the order without uploading finished work images',
  'order_deleted_successfully': 'Order deleted successfully',

  // مفاتيح مكون حذف الطلب
  'confirm_delete_order': 'Confirm Delete Order',
  'warning_delete_order': 'Warning: Delete Order',
  'delete_order_warning_message': 'This action cannot be undone. The order and all associated data will be permanently deleted.',
  'admin_email': 'Admin Email',
  'admin_password': 'Admin Password',
  'enter_admin_email': 'Enter admin email',
  'enter_admin_password': 'Enter admin password',
  'please_fill_all_fields': 'Please fill all fields',
  'email_does_not_match': 'Email does not match the registered admin email',
  'incorrect_password': 'Incorrect password',
  'confirm_delete': 'Confirm Delete',
  'select_delivery_date': 'Select delivery date',
  'busy_date_warning': 'Warning: This date is busy with {count} orders. You can continue but delivery may be delayed.',
  'due_date': 'Due Date',
  'proof_delivery_date': 'Proof Delivery Date',
  'delivered_date': 'Delivered Date',
  'paid': 'Paid',

  // مفاتيح مكون الملاحظات الصوتية
  'start_recording': 'Start Recording',
  'stop_recording': 'Stop Recording',
  'click_to_record_voice_note': 'Click to record a voice note',
  'voice_notes': 'Voice Notes',
  'voice_note': 'Voice Note',
  'microphone_access_error': 'Microphone access error',
  'transcribe_to_text': 'Transcribe to Text',
  'transcribe_all': 'Transcribe All',
  'no_notes_to_transcribe': 'No notes to transcribe',
  'translate': 'Translate',
  'transcription': 'Transcription',
  'translation': 'Translation',
  'select_language': 'Select Language',
  'transcribing': 'Transcribing...',
  'translating': 'Translating...',
  'transcription_error': 'Error transcribing audio',
  'translation_error': 'Error translating text',
  'english': 'English',
  'urdu': 'Urdu',
  'bengali': 'Bengali',
  'edit_transcription': 'Edit Transcription',

  // مفاتيح صفحة العمال
  'worker_added_success': 'Worker added successfully',
  'error_adding_worker': 'Error adding worker',
  'worker_updated_success': 'Worker updated successfully',
  'error_updating_worker': 'Error updating worker',
  'worker_deleted_success': 'Worker deleted successfully',
  'worker_deactivated': 'Worker deactivated',
  'worker_activated': 'Worker activated',
  'adding': 'Adding...',
  'add_worker': 'Add Worker',
  'active': 'Active',
  'inactive': 'Inactive',
  'save_changes': 'Save Changes',
  'search_workers_placeholder': 'Search workers...',
  'workers_management': 'Workers Management',
  'view_manage_team': 'View and manage the tailoring workshop team',
  'add_new_worker': 'Add New Worker',
  'add_new_worker_form': 'Add New Worker',
  'full_name_required': 'Full Name *',
  'email_required': 'Email *',
  'password_required': 'Password *',
  'phone_required_worker': 'Phone Number *',
  'enter_full_name': 'Enter full name',
  'enter_email': 'Enter email',
  'enter_password': 'Enter password',
  'specialty_required': 'Specialty *',
  'specialty_example': 'Example: Evening dress tailoring',
  'edit_worker': 'Edit Worker',

  'leave_empty_no_change': 'Leave empty if you don\'t want to change it',
  'no_workers': 'No workers',
  'no_workers_found': 'No workers found matching the search criteria',
  'joined_on': 'Joined on',
  'total_workers': 'Total Workers',
  'active_workers': 'Active Workers',
  'total_completed_orders': 'Total Completed Orders',
  'confirm_delete_worker': 'Are you sure you want to delete this worker?',

  // مفاتيح صفحة التقارير
  'engagement_dress': 'Engagement Dress',
  'casual_dress': 'Casual Dress',
  'other': 'Other',
  'this_week': 'This Week',

  // Salary Dashboard Translations - English
  'salary_dashboard_title': 'Tailoring Worker Salaries',
  'salary_dashboard_desc': 'Fixed salary and piece-rate payroll in one monthly dashboard.',
  'payroll_type_label': 'Payroll Type',
  'base_salary_label': 'Base Salary',
  'overtime_hours_label': 'Overtime Hours',
  'overtime_value_label': 'Overtime Value',
  'piece_work_details_label': 'Piece Work Details',
  'small_advances_label': 'Small Advances',
  'large_debt_label': 'Large Debt / Repayment',
  'net_salary_label': 'Net Salary',
  'actions_label': 'Actions',
  'fixed_salary_option': 'Fixed Salary',
  'piece_rate_option': 'Piece Rate',
  'view_items_button': 'View Items',
  'repay_button': 'Repay',
  'settle_salary_button': 'Settle Salary',
  'total_net_salary': 'Total Net Salary (Visible Workers)',
  'add_transaction_title': 'Add Advance or Debt',
  'transaction_type_label': 'Type',
  'small_advance_option': 'Small Advance',
  'large_debt_option': 'Large Debt',
  'amount_label': 'Amount',
  'date_label': 'Date',
  'note_label': 'Note (Optional)',
  'add_transaction_button': 'Add Transaction',
  'saving': 'Saving...',
  'no_workers_found_salary': 'No workers found',
  'loading_salary_dashboard': 'Loading salary dashboard...',
  'salary_settlement_success': 'Salary settlement saved successfully.',
  'salary_settlement_error': 'Failed to save salary settlement.',
  'transaction_success': 'Transaction saved successfully.',
  'transaction_error': 'Failed to save transaction.',
  'select_worker_alert': 'Select a worker first.',
  'invalid_amount_alert': 'Enter a valid amount greater than zero.',
  'negative_net_salary_alert': 'Net salary is negative. Please adjust inputs before saving.',
  'piece_rate_items_modal_title': 'Completed Piece Items',
  'close_button': 'Close',
  'item_title': 'Item Title',
  'price_sar_label': 'Price (SAR)',
  'no_items_for_month': 'No completed items for this month.',

  'switch_to_piece_rate': 'Switch to Piece-rate',
  'switch_to_fixed_salary': 'Switch to Fixed Salary',
  'has_debt_outstanding': 'Has Debt: outstanding',
  'debt_repayment_amount_label': 'Debt Repayment Amount',
  'total_advances_auto_label': 'Total Advances (auto)',
  'total_large_debt_label': 'Total Large Debt',
  'debt_repayment_manual_label': 'Debt Repayment (manual)',
  'remaining_net_salary_label': 'Remaining Net Salary',
  'final_net_salary_label': 'Final Net Salary',
  'total_item_income_label': 'Total Item Income',
  'completed_items_label': 'Completed Items / Dresses',
  'loading_items': 'Loading completed items...',
  'price_wage_placeholder': 'Price / Wage',
  'save_monthly_salary_button': 'Save Monthly Salary',
  'this_month': 'This Month',
  'this_quarter': 'This Quarter',
  'this_year': 'This Year',
  'reports_analytics': 'Reports & Analytics',
  'comprehensive_analysis': 'Comprehensive Business Performance Analysis',
  'monthly_trend': 'Monthly Trend',
  'revenue_label': 'Revenue',

  // مفاتيح صفحة المواعيد
  'confirmed': 'Confirmed',
  'pm': 'PM',
  'am': 'AM',
  'all_statuses': 'All Statuses',

  // Tailor Dashboard Keys
  'tailor_dashboard_desc': 'Manage and track your tailoring orders',
  'tailor_workspace': 'Tailor Workspace',
  'tailor_workspace_info': 'You can track your current orders, update their status, and view completed orders archive',
  'worker_attendance_title': 'Attendance',
  'worker_attendance_source': 'Synced from attendance records · Riyadh time',
  'worker_attendance_load_error': 'Attendance summary could not be loaded',
  'morning_arrival_time': 'Morning arrival time',
  'not_checked_in_today': 'Not checked in yet',
  'current_month_absence': 'Absences this month',
  'friday_excluded_note': 'Fridays are excluded from absences',
  'today_label': 'Today',
  'days_unit': 'days',
  'refresh': 'Refresh',
  'current_orders': 'Current Orders',
  'view_update_orders': 'View and update status of orders assigned to you',
  'view_completed_archive': 'View archive of orders you completed',

  // Completed Orders Page Keys
  'completed_orders_subtitle': 'Orders you have finished and are ready for delivery',
  'total_completed_orders_label': 'Total Completed Orders:',
  'no_completed_orders_title': 'No Completed Orders',
  'no_completed_orders_message': 'There are no completed orders at the moment.',
  'delivery_success_message': 'Order status updated to "Delivered" successfully',
  'request_alteration': 'Request Alteration',
  'delete_order': 'Delete Order',
  'all_dates': 'All Dates',
  'all_months': 'All Months',
  'current_month_label': 'Current Month',
  'month_orders_label': 'order(s)',
  'completed_at_label': 'Completed on',
  'worker_completed_at_label': 'Work Completion Date',
  'month_filter_label': 'Month Filter',
  'orders_of_month': 'Orders of',
  'today': 'Today',
  'tomorrow': 'Tomorrow',
  'appointments_management': 'Appointments Management',
  'view_manage_appointments': 'View and manage all tailoring appointments',
  'book_new_appointment': 'Book New Appointment',
  'search_appointments_placeholder': 'Search appointments...',
  'no_appointments': 'No appointments',
  'no_appointments_found': 'No appointments found matching the search criteria',
  'created_on': 'Created on',
  'confirm_appointment': 'Confirm Appointment',
  'cancel_appointment': 'Cancel Appointment',
  'total_appointments': 'Total Appointments',
  'edit_appointment': 'Edit Appointment',
  'confirm_delete_appointment_message': 'Are you sure you want to delete this appointment? This action cannot be undone.',
  'yes_delete': 'Yes, Delete',
  'deleting': 'Deleting...',

  'email_placeholder': 'Example: example@email.com',
  'customer_name_placeholder': 'Example: Ahmad Mohammed',

  'notes_placeholder': 'Any additional notes...',
  'service_type': 'Service Type',
  'customer_name': 'Customer Name',
  'date_time': 'Date & Time',

  // مفاتيح مكونات إضافية
  'of': 'of',
  'images_text': 'images',

  // مفاتيح مكونات الصور والتحميل
  'max_images_reached': 'Maximum images reached',
  'drop_images_here': 'Drop images here',
  'click_or_drag_images': 'Click or drag images here',
  'image_upload_format': 'PNG, JPG, JPEG up to 5MB',
  'max_images_text': 'Maximum',
  'order_label': 'Order',
  'for_client': 'For client',

  // مفاتيح مكون تعديل الطلب
  'edit_order': 'Edit Order',
  'price_sar_required': 'Price (SAR) *',
  'status_pending': 'Pending',
  'status_in_progress': 'In Progress',

  'status_delivered': 'Delivered',
  'status_cancelled': 'Cancelled',

  // نصوص الفوتر
  'home': 'Home',
  'track_order': 'Track Order',
  'fabrics': 'Fabrics',
  'contact_us': 'Contact Us',
  'yasmin_alsham': 'Yasmin Alsham',
  'custom_dress_tailoring': 'Custom Dress Tailoring',

  // مفاتيح مكون تفاصيل الطلب (OrderModal)
  'order_details': 'Order Details',
  'alterations_section_title': 'Alterations',
  'first_proof_alteration': 'First proof alteration',
  'second_proof_alteration': 'Second proof alteration',
  'post_delivery_alteration': 'Post-delivery alteration',
  'post_delivery_alterations_count': 'Post-delivery alterations',
  'order_without_alteration': 'No alterations needed',
  'translate_alteration_text': 'Translate to Hindi',
  'restore_alteration_text': 'Restore original text',
  'loading_alterations': 'Loading alterations...',
  'alterations_load_error': 'Unable to load alterations for this order.',
  'alteration_translation_error': 'Could not translate the alteration text. Please try again.',
  'alteration_translation_save_error': 'The translation was created but could not be saved. Try again to save the same version.',
  'alteration_text_not_available': 'No written text is available for this alteration.',
  'alteration_photos_label': 'Alteration photos',
  'alteration_photo_alt': 'Alteration photo {number}',
  'open_alteration_photo': 'View alteration photo {number} full size',
  'alteration_status_pending': 'Pending',
  'alteration_status_in_progress': 'In progress',
  'alteration_status_completed': 'Completed',
  'alteration_status_delivered': 'Delivered',
  'alteration_status_cancelled': 'Cancelled',
  'alteration_error_reason': 'Error reason',
  'alteration_error_tailor_error': 'Tailor error',
  'alteration_error_cutter_error': 'Cutter error',
  'alteration_error_measurement_error': 'Measurement error',
  'alteration_error_customer_error': 'Customer error',
  'alteration_error_reception_error': 'Reception error',
  'alteration_error_other_error': 'Other error',
  'customer_information': 'Customer Information',
  'order_date': 'Order Date',
  'delivery_date': 'Delivery Date',
  'assigned_worker': 'Worker',
  'chest_bust': 'Chest/Bust',
  'advanced_tailoring_measurements': 'Advanced Tailoring Measurements',
  'additional_measurements': 'Additional Measurements',
  'dress_length': 'Dress Length',
  'shoulder_width': 'Shoulder Width',
  'sleeve_length_old': 'Sleeve Length',
  'design_image_alt': 'Design Image',
  'design_links': 'Design Links',
  'design_links_label': 'Design Links (Optional)',
  'design_links_placeholder': 'Add reference design links here, one link per line...',
  'completed_work_images': 'Completed Work Images',
  'completed_work_description': 'Completed work images uploaded by worker',
  'completed_work_image_alt': 'Completed Work Image',

  // مفاتيح صفحة الطلبات
  'view_manage_orders': 'View and manage all orders',
  'order_date_label': 'Order Date',
  'delivery_date_label': 'Delivery Date',
  'worker_label': 'Worker',
  'fabric_label': 'Fabric:',
  'notes_label': 'Notes',

  // مفاتيح صفحة العمال
  'experience_years': 'Years of Experience',

  // مفاتيح إضافية للوحة التحكم
  'control_panel': 'Control Panel',
  'completed_orders_management': 'Completed Orders',
  'delivered_orders_management': 'Delivered Orders',
  'ready_designs_management': 'Ready Designs Management',
  'fabrics_management': 'Fabrics Management',
  'orders_in_progress': 'Orders in Progress',
  'appointments_today': 'Today\'s Appointments',
  'finished_orders': 'Finished Orders',

  'new_orders': 'New Orders',
  'delivered_orders_count': 'Delivered Orders',
  'view_all_orders_button': 'All Orders',

  'productive_day_message': 'We wish you a productive day in completing orders',

  // مفاتيح صفحة طلباتي المكتملة للعامل
  'worker_completed_orders_desc': 'Orders you have completed',
  'search_by_name_or_order': 'Search by client name or order number...',
  'no_completed_orders': 'No completed orders',
  'no_completed_orders_desc': 'You have not completed any orders yet. Orders will appear here after completion.',
  'photos': 'photos',
  'order_info': 'Order Information',

  'edit_measurements': 'Edit Measurements',
  'measurements_modal_title': 'Measurements',
  'save_measurements': 'Save Measurements',
  'measurement_sh': 'Shoulder (sh)',
  'measurement_shr': 'Shoulder Circumference (shr)',
  'measurement_ch': 'Chest (ch)',
  'measurement_w': 'Waist (w)',
  'measurement_hi': 'Hips (hi)',
  'measurement_p': 'Dart Length (p)',
  'measurement_L': 'Bodice Length (L)',
  'measurement_v': 'Neckline (v)',
  'measurement_HF': 'Armpit (HF)',
  'measurement_K': 'Sleeve Length (K)',
  'measurement_S': 'Forearm (S)',
  'measurement_S1': 'Cuff (S1)',
  'measurement_L_front': 'Front Length (L)',
  'measurement_LB': 'Back Length (LB)',
  'measurement_additional_notes': 'Additional Measurements',
  'additional_measurements_placeholder': 'Enter any additional measurements here...',
  'measurements_saved_successfully': 'Measurements saved successfully',
  'measurements_save_error': 'Error saving measurements',

  // Design Comments section keys for OrderModal
  'view_front': 'Front',
  'view_back': 'Back',
  'design_comments': 'Design Comments',
  'comment_badge': '{count} comments',
  'comment_default_title': 'Comment',
  'markers_and_drawings': '({markers} markers, {drawings} drawings)',
  'annotations_count': 'Annotations ({count})',
  'stop_audio': 'Stop',
  'play_audio': 'Play Audio',
  'translation_label': 'Translation',
  'marker_without_comment': 'Marker without comment',

  'add': 'Add',
  'primary_fabric': 'Primary Fabric',
  'select_fabric_first': 'Please select a fabric image first or add a secondary fabric',
  'ai_notes_placeholder': 'Any additional instructions or notes for the AI (e.g., I want the dress with long sleeves, add sequins on the bodice...)',

  // AI Design Generation keys (GenerateDesignButton)
  'ai_generate_design': 'Generate Design with AI',
  'generated_designs_count': 'Generated Designs ({count})',
  'all_generated_designs': 'All Generated Designs ({count})',
  'select_fabric_image': 'Select Fabric Image',
  'fabric_image_desc': 'Select one or more order images to use as fabric reference',
  'no_images_available': '(No images available)',
  'add_secondary_fabric': 'Add Secondary Fabric',
  'generate_design': 'Generate Design',
  'select_fabric_location': 'Assign Fabric Placement on Body',
  'fabric_location_desc': 'You can select multiple areas for each fabric',
  'other_location': 'Other location',
  'type_location_placeholder': 'Type location...',
  'skip': 'Skip',
  'generating_design_heading': 'Generating Design',
  'analyzing_design': 'Analyzing design and creating professional description...',
  'generating_image_text': 'Generating design image...',
  'generation_step': 'Step {step} of 2 — You can save the order now and wait',
  'design_generated_success': 'Design generated successfully!',
  'generate_new_design': 'Generate New Design',
  'other_fabric': 'Other fabric',
  'type_fabric_placeholder': 'Type fabric type...',
  'select_action': 'Select',
  'fabric_color': 'Fabric Color',
  'other_color': 'Other color',
  'type_color_placeholder': 'Type color...',
  'download': 'Download',
  'design_number': 'Design {n}',
  'generated_design_alt': 'Generated design {n}',
  'image_alt_number': 'Image {n}',

  // Cartoon design section keys (OrderModal)
  'converting_to_cartoon': 'Converting...',
  'convert_to_cartoon': 'Convert Image to Cartoon',
  'cartoon_design': 'Cartoon Design',
  'print_image': 'Print Image',
  'regenerate': 'Regenerate',
  'no_completed_image_title': 'No Completed Work Image',
  'no_completed_image_desc': 'No completed work image uploaded yet. You can upload an image from your device to use for cartoon conversion.',
  'uploaded_image_alt': 'Uploaded image',
  'convert_to_cartoon_btn': 'Convert to Cartoon',
  'change': 'Change',
  'click_to_upload_dress': 'Click to upload dress image',

  // Management rating section in worker completed orders page
  'management_rating_title': 'Management Rating',
  'piece_wage_label': 'Piece Wage',
  'sar_unit': 'SAR',
  'bonus_label': 'Bonus',
  'rating_out_of': '{rating}/5',
  'visible_salary_total': 'Total Salary',
  'visible_salary_note': 'Pricing shared by management',
  'completed_work_thumbnail_alt': 'Completed work image for order {order}',
  'no_completed_work_image': 'No completed work image',

  'language': 'en'
}

// Hook للترجمة
export function useTranslation() {
  const [language, setLanguage] = useState<'ar' | 'en'>('ar')

  // تحميل اللغة المحفوظة عند بدء التطبيق
  useEffect(() => {
    const savedLanguage = localStorage.getItem('dashboard-language') as 'ar' | 'en'
    if (savedLanguage) {
      setLanguage(savedLanguage)
    }
  }, [])

  // حفظ اللغة عند تغييرها
  const changeLanguage = (newLanguage: 'ar' | 'en') => {
    setLanguage(newLanguage)
    localStorage.setItem('dashboard-language', newLanguage)
  }

  // دالة الترجمة مع دعم المتغيرات
  const t = (key: TranslationKey, variables?: Record<string, any>): string => {
    const translations = language === 'ar' ? arTranslations : enTranslations
    const translation = translations[key]

    if (typeof translation === 'string') {
      // استبدال المتغيرات في النص
      if (variables) {
        let result = translation
        Object.keys(variables).forEach(varKey => {
          result = result.replace(`{${varKey}}`, String(variables[varKey]))
        })
        return result
      }
      return translation
    }

    // إذا لم توجد الترجمة، أرجع المفتاح نفسه
    return key
  }

  // التحقق من اللغة الحالية
  const isArabic = language === 'ar'
  const isEnglish = language === 'en'

  return {
    language,
    changeLanguage,
    t,
    isArabic,
    isEnglish
  }
}
