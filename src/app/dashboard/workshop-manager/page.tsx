...
export default function WorkshopManagerDashboard() {
  ...
  useEffect(() => {
    ...
    // التحقق من الصلاحيات
    if (!isLoading && workerType !== 'workshop_manager' && user.role !== 'admin') {
    ...
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-pink-50 to-purple-50">
        ...
        <div className="w-full max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between py-3 sm:py-0 sm:h-16 gap-3 sm:gap-0">
            ...
            <div className="flex items-center space-x-2 sm:space-x-4 space-x-reverse w-full sm:w-auto">
              <Link
                href="/"
                className="text-pink-600 hover:text-pink-700 transition-colors duration-300 group flex items-center space-x-1 sm:space-x-2 space-x-reverse flex-shrink-0"
              >
                <ArrowRight className="w-4 h-4 sm:w-5 sm:h-5 group-hover:translate-x-1 transition-transform duration-300" />
                <span className="text-xs sm:text-sm font-medium hidden sm:inline">
                  {t('home_page') || 'الصفحة الرئيسية'}
                </span>
              </Link>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-gray-600 text-xs sm:text-sm truncate">
                  {t('workshop_manager_dashboard') || 'لوحة تحكم مدير الورشة'}
                </p>
              </div>
              <span className="px-2 sm:px-3 py-1 bg-gradient-to-r from-blue-100 to-indigo-100 text-blue-700 rounded-full text-xs sm:text-sm font-medium whitespace-nowrap flex-shrink-0">
                {t('workshop_manager') || 'مدير الورشة'}
              </span>
            </div>
      ...
      <main className="w-full max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        >
          <div className="text-center sm:text-right">
            ...
            <p className="text-sm sm:text-base md:text-lg text-gray-600">
              {t('workshop_manager_description') || 'إدارة ومتابعة الطلبات في الورشة'}
            </p>
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-4 sm:p-6 mb-6 sm:mb-8"
        >
          <div className="flex items-start space-x-3 sm:space-x-4 space-x-reverse">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-base sm:text-lg md:text-xl font-semibold text-blue-800 mb-1 sm:mb-2">
                {t('workshop_manager') || 'مدير الورشة'}
              </h3>
              <p className="text-xs sm:text-sm md:text-base text-blue-600">
                {t('workshop_manager_info') || 'يمكنك متابعة جميع الطلبات في الورشة: الطلبات الحديثة، المكتملة، والمسلمة'}
              </p>
            </div>
          </div>
        </motion.div>
              ...
              <div className="flex flex-col items-center text-center space-y-3 sm:space-y-4">
                ...
                <div>
                  <h4 className="text-base sm:text-lg font-bold text-blue-800 mb-1 sm:mb-2">
                    {t('recent_orders') || 'الطلبات الحديثة'}
                  </h4>
                  <p className="text-xs sm:text-sm text-blue-600">
                    {t('view_track_orders') || 'عرض ومتابعة الطلبات الجارية'}
                  </p>
                </div>
              </div>
            </Link>
