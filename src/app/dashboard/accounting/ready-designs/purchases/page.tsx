...
function ReadyDesignsPurchasesContent() {
  ...
  const loadExpenses = async () => {
    setLoading(true)
    try {
      const data = await getExpenses('ready_designs', 'material')
      setExpenses(data)
    } catch (error) {
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-500">شراء الفساتين الجاهزة</p>
              </div>
            </div>
          </div>
        </motion.div>
...
export default function ReadyDesignsPurchasesPage() {
  return (
    <ProtectedRoute requiredRole="admin">
      <ReadyDesignsPurchasesContent />
    </ProtectedRoute>
  )
}
