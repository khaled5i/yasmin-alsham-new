...
export default function Header() {
  ...
  const shouldHideHeader = isHomePage && isScrolled && !isMenuOpen && isMobile
  ...
  const getHeaderClasses = () => {
    ...
    if (isScrolled) {
      // بعد السكرول في الصفحة الرئيسية: شفاف بالكامل (إخفاء الشريط)
      return 'bg-transparent border-b-0 shadow-none pointer-events-none'
    }
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 lg:h-20 relative">
            ...
            <motion.button
              ...
              onClick={toggleMenu}
              className={`lg:hidden p-2 rounded-lg transition-all duration-300 ${shouldHideHeader
                ? 'invisible opacity-0 pointer-events-none'
                : isHomePage && !isScrolled && !isMenuOpen && !(typeof window !== 'undefined' && (window as any).Capacitor)
            ...
            {/* الشعار - يختفي على الجوال عند التمرير في الصفحة الرئيسية */}
            <motion.div
              initial={{ opacity: 0, y: -10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5 }}
              className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 lg:static lg:translate-x-0 lg:translate-y-0 cursor-pointer hover:opacity-80 transition-all duration-300 lg:flex lg:items-center lg:space-x-2 lg:space-x-reverse ${shouldHideHeader ? 'max-lg:invisible max-lg:opacity-0 max-lg:pointer-events-none' : ''}`}
          ...
          <motion.div
            ...
            className="lg:hidden overflow-hidden bg-white border-t border-pink-100 pointer-events-auto"
          >
