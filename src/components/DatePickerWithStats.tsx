...
const HIJRI_MONTH_COLORS = {
  first: { text: '#7c3aed', tailwind: 'text-purple-700' },  // بنفسجي للشهر الأول
  second: { text: '#0d9488', tailwind: 'text-teal-600' },   // زمردي للشهر الثاني
}
...
export default function DatePickerWithStats({
  ...
  const renderDayContents = (day: number, date: Date) => {
    ...
    let hijriDayColor = '#6b7280' // رمادي افتراضي
    if (hijriMonthsInView.length > 1) {
      // عند وجود شهرين: لوّن كل رقم بلون شهره
    ...
    return (
      <div className="relative w-full h-full flex flex-col items-center justify-center py-0.5">
        ...
        {count > 0 && (
          <span
            className={`cal-order-count text-[14px] font-bold leading-none ${isOverloaded
              ? 'text-red-700'
              : 'text-pink-700'
              }`}
          >
      ...
      <div className="flex items-center justify-between px-2 py-2">
        ...
        <div className="text-center">
          <div className="text-sm font-bold text-pink-900">
            {gregorianMonth} <span className="font-normal text-pink-600">({monthNumber})</span>
          </div>
