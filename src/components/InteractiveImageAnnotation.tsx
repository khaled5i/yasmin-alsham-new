...
const InteractiveImageAnnotation = forwardRef<InteractiveImageAnnotationRef, InteractiveImageAnnotationProps>(({
    ...
    try {
      ...
      annotations
        ...
        .forEach((annotation) => {
          ...
          // اتجاه النص (يطابق dir="auto" في HTML للترجمة، أو dir="rtl" للعربي)
          const isRtl = isRtlText(rawText)
          ...
          // قياس أعرض سطر (مطابق لـ <p> shrunk-to-content مع max-w-[200px])
          ctx.font = `${fontSize}px Cairo, Arial, sans-serif`
          let maxLineWidth = 0
          lines.forEach(line => {
          ...
          // RTL (عربي): textX هو الحافة اليسرى → [النص يمين-محاذاة][gap][الرقم على اليمين]
          // LTR (لاتيني): textX هو الحافة اليسرى → [الرقم][gap][النص يسار-محاذاة]
  ...
  const tryFinalizeAnnotation = () => {
    ...
    reader.onloadend = () => {
      ...
      if (annotationId === '__design_summary__') {
        ...
        // التحويل من الملف المسجّل (async) — نُعيد ترميزه إلى WAV نظيف أولاً
        // لأن webm الخام من MediaRecorder يجعل معالج Soniox يتوقّف (timed out)
        {
          ...
          ;(async () => {
            try {
              ...
              console.error('Summary async transcription failed:', err)
            } finally {
  ...
  const startRecording = async (annotationId: string) => {
    try {
      ...
      // ملخص التصميم يستخدم التحويل من الملف المسجل (async) فقط لدقة أعلى،
      // لذا نتخطّى الاتصال اللحظي ونترك المنطق يسلك مسار async في tryFinalizeAnnotation
