...
export default function AttendanceMonitoringDashboard({ embeddedWorkerId }: AttendanceMonitoringDashboardProps = {}) {
  ...
  const loadAttendance = useCallback(async () => {
    ...
    try {
      ...
      setWorkers(workersResult.filter((worker) => (
        worker.user?.is_active !== false
        && (!embeddedWorkerId || worker.id === embeddedWorkerId)
      )))
  ...
  }, [dateKey, embeddedWorkerId, isAuthorized])
  ...
  if (!user || permissionsLoading || !isAuthorized) {
    return <AttendanceLoading embedded={Boolean(embeddedWorkerId)} />
  }

  return (
    <div className={embeddedWorkerId ? 'text-slate-900' : 'min-h-screen bg-[#f4faf9] text-slate-900'} dir="rtl">
      {!embeddedWorkerId && (
      ...
      <main className={embeddedWorkerId ? 'space-y-4' : 'mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 lg:py-8'}>
        {embeddedWorkerId ? (
          <section className="overflow-hidden rounded-2xl border border-teal-200 bg-[linear-gradient(120deg,#f0fdfa_0%,#ffffff_55%,#ecfeff_100%)] p-4 shadow-sm sm:p-5">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-teal-950 px-2.5 py-1 text-xs font-black text-white">
                    <Fingerprint className="h-3.5 w-3.5" />
                    نافذة الحضور المرتبطة
                  </span>
                  <span className="rounded-full border border-teal-200 bg-white px-2.5 py-1 text-xs font-bold text-teal-800">
                    توقيت الرياض
                  </span>
                </div>
                <h2 className="mt-2 text-lg font-black text-slate-950 sm:text-xl">الحضور والانصراف للعامل</h2>
                <p className="mt-1 text-xs leading-5 text-slate-500 sm:text-sm">
                  نفس سجل الحضور الأساسي وحسابات التأخير والنقص والعمل الإضافي، مع عرض هذا العامل فقط.
                </p>
              </div>
              <div className="grid grid-cols-2 gap-2 sm:flex sm:items-end">
                {view === 'daily' && (
                  <label className="col-span-2 block sm:w-44">
                    <span className="mb-1 block text-xs font-black text-slate-600">اليوم</span>
                    <input
                      type="date"
                      value={dateKey}
                ...
                <button
                  ...
                  onClick={() => void loadAttendance()}
                  disabled={isLoading}
                  className="col-span-2 inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border border-teal-200 bg-white px-4 text-sm font-black text-teal-800 transition hover:bg-teal-50 disabled:cursor-wait disabled:opacity-60 sm:col-span-1"
                >
                  <RefreshCw className={`h-4 w-4 ${isLoading ? 'animate-spin' : ''}`} />
                  تحديث
                </button>
        ...
        {(!embeddedWorkerId || activeSuspendedWorkers.length > 0 || (view === 'daily' && historicalSuspendedWorkers.length > 0)) && (
        ...
        {view === 'daily' ? (
          ...
          <div className="flex flex-col gap-2 border-b border-slate-100 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <div>
              <h3 className="font-black text-slate-900">{embeddedWorkerId ? 'تفاصيل حضور العامل' : 'سجل العمال اليومي'}</h3>
              <p className="mt-1 text-xs text-slate-500">افتح السجل لرؤية تفاصيل التأخير والصلاة والعمل الإضافي.</p>
            </div>
...
function AttendanceLoading({ embedded = false }: { embedded?: boolean }) {
  return (
    <div className={embedded ? 'grid min-h-48 place-items-center rounded-2xl border border-teal-100 bg-teal-50/50' : 'grid min-h-screen place-items-center bg-[#f4faf9]'} dir="rtl">
